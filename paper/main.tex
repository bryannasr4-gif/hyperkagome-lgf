...
\title[Lattice Green's function of the hyperkagome lattice]{Lattice Green's function of the hyperkagome lattice:\\
...
\address{LPTMC, UMR 7600 CNRS, Sorbonne Universit\'e, 4 Place Jussieu, 75252 Paris Cedex 05, France}

\date{\today}

\subjclass[2020]{Primary 34M15, 11F03; Secondary 82B20, 12H05, 11F11}
\keywords{Lattice Green's function, hyperkagome lattice, Picard--Fuchs operator,
differential Galois theory, symmetric square, modular curve, Fricke group,
Hauptmodul, quasimodular forms, creative telescoping}

\begin{abstract}
The lattice Green's functions (LGFs) of the simple-cubic, body- and face-centred cubic lattices are
classical: each is the symmetric square of a second-order operator and admits a closed form in
complete elliptic integrals. The three-dimensional \emph{hyperkagome} lattice---the corner-sharing
triangle network realised by the Ir sublattice of the spin-liquid candidate
Na$_4$Ir$_3$O$_8$---has resisted such a treatment: Varma and Monien [Phys.\ Rev.\ E \textbf{87},
032109 (2013)] reduced its density of states to a threefold integral they ``found no way to exactly
solve.'' We determine its analytic structure and prove that it is \emph{modular}. From exact lattice
moments we obtain a third-order Picard--Fuchs operator $M$ of degree fifteen annihilating the LGF,
after removing the trivial flat-band pole and using the exact $E=1$ spectral symmetry. It is
irreducible over $\overline{\mathbb{Q}}(t)$ and is not a \emph{literal} symmetric square, yet
$\mathrm{Sym}^2(M)$ possesses an explicit rational solution: the differential Galois group is the
orthogonal group $\mathrm{O}(3,\mathbb{C})$, and $M$ is projectively---through an order-two
differential intertwiner---the symmetric square of an explicit second-order operator $V_2$. Our main
result is that $V_2$ \emph{uniformizes} the genus-zero modular curve $X(\Gamma_0(30)^+)$: with an
explicit level-30 eta quotient $u$, the variable $t=u/(u^2+7u+1)$ generates its function field, and
the Schwarzian identity $\{\tau,t\}=2Q_V(t)$ holds exactly---proven by a pole-degree bound, not
checked numerically. The hyperkagome LGF is therefore modular at level $30=2\cdot3\cdot5$, to our
knowledge the first LGF realised at a modular level with three distinct prime factors; this
establishes the closed form whose existence Varma and Monien conjectured. Its weight-two period is
obtained in explicit closed form, a depth-one quasimodular form twisted by the determinant
character, provably \emph{not} a modular form times an algebraic function at any weight. That $M$ is
the \emph{minimal} annihilator is established to the guess-and-verify standard of this field; an
unconditional creative-telescoping proof of minimality remains open.
\end{abstract}
...
\label{eq:schwarz}
\end{equation}
holds exactly~\cite{AbdelazizMaillard2017,AbdelazizKoutschanMaillard2017}. The Schwarzian route from an
...
in~\cite{AbdelazizMaillard2017,AbdelazizKoutschanMaillard2017}; \eqref{eq:schwarz} is an instance in
...
of~\eqref{eq:schwarz} is the a-priori pole-degree bound established below, which converts such a
...
\emph{Proof.} Three exact steps, certified in
...
\bibitem{AbdelazizKoutschanMaillard2017} Y.~Abdelaziz, C.~Koutschan, and J.-M.~Maillard, ``Schwarzian conditions for linear differential operators with selected differential Galois groups,'' J.\ Phys.\ A: Math.\ Theor.\ \textbf{50}, 465201 (2017); arXiv:1706.07423.
\bibitem{FordMcKayNorton1994} D.~Ford, J.~McKay, and S.~P.~Norton, ``More on replicable functions,'' Commun.\ Algebra \textbf{22}, 5175 (1994). See also OEIS \href{https://oeis.org/A205826}{A205826} (class $30\mathrm{A}$) and \href{https://oeis.org/A058613}{A058613} (class $30\mathrm{B}$).
\bibitem{LianWiczer2006} B.~H.~Lian and J.~L.~Wiczer, ``Genus zero modular functions,'' arXiv:math/0611291.
\end{thebibliography}
