...
variable $t=u/(u^2+7u+1)$ generates the genus-zero function field of $X(\Gamma_0(30)^+)$ (equivalently
$1/t=u+7+1/u$), and the projective
...
series), exactly as the orthogonal Galois group demands. The associated $\,_2F_1$ representation follows via an
explicit algebraic pullback, which we show is solvable in radicals over $\mathbb{Q}(t)$, with Galois group the
Atkin--Lehner group $(\mathbb{Z}/2)^3$ of the level. We further reduce the LGF, exactly, to a classical two-parameter Watson
...
pullback), verified in exact arithmetic through the bridge identity $f_0^2=P(y_0)$ with $V_2(f_0)=0$ on every
...
algebraic of degree eight over $\mathbb{Q}(t)$ and solvable in radicals (the Proposition of
Sec.~\ref{sec:modular})---though the modular parametrization itself remains the closed form in its natural
coordinates.
...
is one for $\Gamma_0(30)^+$. We reserve the word here for the pullback $1728/j$ that arises as the argument of
the $\,_2F_1$ in an Assis--van~Hoeij--Maillard-style representation~\cite{AVM2016}, and refer to the explicit
...
\emph{Theorem.} $1/t$ generates the genus-zero function field of $X(\Gamma_0(30)^+)$ (so $t$ is a degree-one
...
\label{eq:schwarz}
\end{equation}
holds exactly~\cite{AbdelazizMaillard2017}. Equivalently: the ratio of two solutions of $w''+Q_Vw=0$ inverts the parametrization $t(\tau)$, $V_2$ is the
...
Green's function realised at a level with three distinct prime factors. The
...
\emph{Proposition (the pullback in radicals).} The pullback $H=1728/j$, the argument of the
...
the $\,_2F_1$ pullback is solvable in radicals. The product of the three radicands is $\big((1-t)\,v\big)^2$:
the determinant-character twist $v$ of Eq.~\eqref{eq:twist} is, up to sign and the rational factor $1-t$, the
...
index-two subfield of \eqref{eq:radfield} (M.~van~Hoeij and one of us, J.-M.M.); its analytic branch
$H=256t-25600t^2+\cdots$ is verified against the certified $Q_V$ through the Schwarzian pullback identity to
order $t^{96}$.}

\emph{Proof.} The coefficients of $P_H$, an explicit radical expression for $H$ due to
M.~van~Hoeij (private communication, July 2026), and the machine certificate are public
(\texttt{numerics/pullback\_data.json}, \texttt{numerics/certify\_pullback.py}). The certificate proves four
...
that no point above $j=0$ retains an order-three stabiliser, so all $72/3=24$ of them are ramified of index
...
as many zeros as poles, its order of vanishing at the cusp---where $q$ is a uniformiser, $\Gamma_0(30)$ having
...
That field is $\mathbb{Q}(X_0(30))$: by the theorem of this section $\mathbb{Q}(t)$ is the fixed field of
...
generated by the simultaneous sign flip and has order two, not four. \hfill$\square$
...
Sec.~\ref{sec:modular}. Write $W=q\,\rmd t/\rmd q$ for the nome derivative of the modular function $t$ of
...
\label{eq:Delta}
\end{equation}
The identity \eqref{eq:phiid} is proven by differentiation---it reduces to \eqref{eq:y0closed} as
...
\emph{Theorem ($\Phi$ lies outside the quasimodular module of $y_0$).}
\begin{equation}
...
\emph{Proof.} Three exact steps, certified in \texttt{numerics/certify\_phi\_obstruction.py}
(\texttt{CERTIFICATE\_phi\_obstruction.txt}). (a) For any nonzero $P=S_0+S_1\,\rmd/\rmd t+S_2\,
...
symmetric square of the standard representation of $\mathcal{G}=\mathrm{SL}(2,\mathbb{C})$ (the
Lemma of Sec.~\ref{sec:y0closed}). And an algebraic value is rational, by connectedness of
...
$\Phi=C+(S_0W+S_1W'+S_2W'')/v$ with rational $C,S_i$. Differentiating, eliminating $W'''$ by
$\widetilde N(W)=0$, and comparing with $\Phi'=2y_0$ coefficient-wise in the independent triple
$W,W',W''$ forces $C'=0$ (by the last sentence of (a)), $S_1=-\rmd_vS_2$ and
...
four-dimensional linear system is inconsistent, exactly over $\mathbb{Q}$. \hfill$\square$
...
\label{eq:dzG}
\end{equation}
the object the closed form \eqref{eq:y0closed} evaluates exactly is this derivative combination,
...
components on the twist cover of Eq.~\eqref{eq:twist}---we leave open.
...
second-neighbour literature (Morita and Horiguchi~\cite{MoritaHoriguchi1971}; the extended Watson integrals of
...
arithmetic in the repository. % TODO(Bryan): the explicit classical bcc(1,2) evaluation along this curve is still
% to be run down in the Morita--Horiguchi / Glasser--Zucker / Delves--Joyce literature; references now cited.
}
...
(Sec.~\ref{sec:modular}) to be the uniformizing operator of the modular curve $X(\Gamma_0(30)^+)$: $t$
generates its function field, given exactly by the level-30 eta quotient \eqref{eq:etaquot}--\eqref{eq:hauptt},
and the LGF is modular at level $30=2\cdot3\cdot5$. Equivalently (this section) it is the classical generalized bcc$(1,2)$
...
explicit closed form. The result is also delimited exactly: the derivative
...
function $\Delta$ of Eq.~\eqref{eq:Delta} (Sec.~\ref{sec:phiobs}). Constructively, $M$ is an irreducible
...
the explicit $V_2$ (private communication, July 2026) and certified in Sec.~\ref{sec:modular}.
...
\bibitem{GlasserZucker1977} M.~L.~Glasser and I.~J.~Zucker, ``Extended Watson integrals for the cubic lattices,'' Proc.\ Natl.\ Acad.\ Sci.\ USA \textbf{74}, 1800 (1977).
\bibitem{MoritaHoriguchi1971} T.~Morita and T.~Horiguchi, ``Calculation of the lattice Green's function for the bcc, fcc, and rectangular lattices,'' J.\ Math.\ Phys.\ \textbf{12}, 986 (1971).
...
\bibitem{HMZ2025} S.~Hassani, J.-M.~Maillard, and N.~Zenine, ``On the diagonals of rational functions: the minimal number of variables,'' arXiv:2502.05543 (2025).
