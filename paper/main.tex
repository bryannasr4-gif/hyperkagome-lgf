...
\author{\fnms{Jean-Marie} \snm{Maillard}%
        \ead[label=e2]{maillard@lptmc.jussieu.fr}%
        \ead[label=e3]{jean-marie.maillard@sorbonne-universite.fr}}
...
to solve exactly. From exact lattice moments we obtain an irreducible third-order Picard--Fuchs
operator annihilating the Green's function. It is not a literal symmetric square, yet its symmetric
...
the underlying second-order operator uniformizes the genus-zero modular curve of the Fricke group of
level thirty, an explicit eta quotient parametrizing the natural variable; the Schwarzian identity is
proved by a pole-degree bound, not checked numerically. The hyperkagome Green's function is therefore
modular at level thirty, which establishes the closed form Varma and Monien conjectured. Its
weight-two period is obtained explicitly, as a depth-one quasimodular form provably not a modular
...
energy origin at the band center. Diagonalizing $H(\bm k)$ over the Brillouin zone yields the spectrum shown in
...
studied in~\cite{AbdelazizMaillard2017,AbdelazizKoutschanMaillard2017}. The Schwarzian route from an
...
$v$ be the twist unit of Eq.~\eqref{eq:twist} in the branch $v(0)=+1$.
...
The identity is proven, not fitted, in five steps.
...
This realization anchors the modular closed form of Sec.~\ref{sec:modular} on the lattice side: $I(w;a,b)$ is
...
Sec.~\ref{sec:modular}, independently of any particular literature evaluation. The remaining task is to specialize such an evaluation to the curve above, or to integrate
...
period $y_0$ itself is exhibited explicitly in Sec.~\ref{sec:y0closed} as a depth-one quasimodular form on
...
The result is directly relevant to the modeling of Na$_4$Ir$_3$O$_8$ and related hyperkagome magnets, where the
...
\bibitem{Watson1939} G.~N.~Watson, ``Three triple integrals,'' Q.\ J.\ Math.\ Oxford Ser.\ \textbf{10}, 266--276 (1939).
...
\bibitem{HMZ2025} S.~Hassani, J.-M.~Maillard, and N.~Zenine, ``On the diagonals of rational functions: the minimal number of variables,'' J.\ Phys.\ A: Math.\ Theor.\ \textbf{58}, 255202 (2025); arXiv:2502.05543.
...
\bibitem{Ligozat1975} G.~Ligozat, \emph{Courbes modulaires de genre 1}, Bull.\ Soc.\ Math.\ France, M\'em.\ \textbf{43} (Soci\'et\'e math\'ematique de France, Paris, 1975), 80 pp.
...
\bibitem{Beukers1987} F.~Beukers, ``Irrationality proofs using modular forms,'' in \emph{Journ\'ees arithm\'etiques de Besan\c{c}on}, Ast\'erisque \textbf{147--148}, 271--283 (1987).
