...
determinant character---the derivative term is essential, so the period is quasimodular rather than modular.
...
\label{eq:schwarz}
\end{equation}
holds exactly. Equivalently: the ratio of two solutions of $w''+Q_Vw=0$ inverts the Hauptmodul, $V_2$ is the
...
of \eqref{eq:y0poly} through degree $40$ agrees. Whether $y_0$ admits a representation as a pure
eta quotient or as (weight-two form)$\times$(algebraic function)---where an algebraic factor
could itself carry the missing half-integer---is a stronger, global question; the structure of
the intertwiner points against it, but we do not claim an all-degrees proof here. This refines
consequence (ii) of Sec.~\ref{sec:modular}.
% TODO: upgrade the eta-quotient / weight-two-times-algebraic exclusion to a theorem via the
% lemma rho_1 != 0 => W'/W algebraic, contradicting the infinite monodromy orbit of the
% log-derivative (needs only G^0 = SO(3)).
...
by the determinant character. Every structural statement here is a rigorous consequence of $M$ and of the
