...
genus-zero modular curve $X(\Gamma_0(30)^+)$, the natural variable $t$ generates its function field through an
explicit level-30 eta-quotient parametrization, and the hyperkagome LGF is modular at level $30=2\cdot3\cdot5$---to our knowledge the first
...
$\pm\tfrac12$ permits a logarithm; direct Lorentzian broadening of the Brillouin-zone spectrum is consistent
with a logarithmically divergent $\mathrm{Im}\,G$, and, as the $E=1$ symmetry requires, gives the same
divergence at $E=0$, which is the same point $t=1$.) From the exponent $-\tfrac12$ at
